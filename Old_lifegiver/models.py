...
class Donor(db.Model, UserMixin):
    ...
    # try making the nullables True

    phone_number = db.Column(db.String(45), nullable=False)
    blood_type = blood_type = db.Column(db.Enum('O-', 'O+', 'A-', 'A+', 'B-', 'B+', 'AB-', 'AB+',), nullable=False)
    ...
    donor_donations = db.relationship("Donation", back_populates="donor") #('UserDonation', backref='donor', lazy=True)
...
#    email = db.Column(db.String(125), unique=True, nullable=False)
    ...
    hospital_donations = db.relationship('UserDonation', backref='hospital_user_donation', lazy=True)
    ...
    emergency_requests = db.relationship('UrgentRequest', backref='hospital_urgent_request', lazy=True)
...
class UserDonation(db.Model):
    ...
    donors = db.relationship("Donor", back_populates="donor_donations")
